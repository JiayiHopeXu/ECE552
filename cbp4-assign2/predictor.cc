...
short Bi_Model[32768];   
tage_entry T[5][128];       // 4,8,16,32,64 bits history
...
for(int j=4; j>=0; j--)
{
   ...
   if (j==4)   //64
   index = (((bhist>>32)<<32)+(PC^(bhist&0xffffffff)));
else if (j==3)  //32
    ...
    for( int i=0; i<128;i++)
  {
    if(T[j][i].tag==index && (((T[j][i].u)>>1)&0x1)==1)
    {
       //printf("j=%d, index=%lld\n", j,index);
        ...
        //printf("1");
        if(((T[j][i].ctr>>1)&0x1)==1)
        return TAKEN;
       else if (((T[j][i].ctr>>1)&0x1)==0)
        return NOT_TAKEN;
    }
  }
}

unsigned int BiModelIndex = (bhist^PC)&0x7fff;
//printf("2");
...
for(int j=0; j<=4; j++)
{
   ...
   for( int i=0; i<128;i++)
   {
    if(T[j][i].tag==index)// && (((T[j][i].u)>>1)&0x1)==1)
    {
       ...
       if(resolveDir == ((T[j][i].ctr>>1)&0x1))
       {
           //printf("a");
           used_prediction_entry.correctness = 1;
          T[j][i].u = min(T[j][i].u+1,3);
          if(resolveDir == TAKEN)
             T[j][i].ctr = min(T[j][i].ctr+1,3);
	  else
...
if (used_prediction_entry.correctness == -1){
   ...
   for (int a=used_prediction_entry.J+1; a<5; a++)
   {
       for (int b=0; b<128; b++)
       {
           if(T[a][b].u==0)
